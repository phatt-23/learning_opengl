...
export class ShaderProgram {
    ...
    static auto compileShader(const GLuint shaderType, const std::string& source) -> GLuint {
    ...
    static auto createShaderProgram(const ShaderProgramSource& shaderSource) -> GLuint {
...
public:
    explicit ShaderProgram(std::string shaderFilePath)
        ...
        this->shaderProgramID = createShaderProgram(sources);
    }
    ...
    auto getUniformLocation(const std::string& variableName) -> GLint {
        ...
        if (location == -1) {
            ...
            std::cerr << "Could not get location of uniform variable called '" << variableName
                      << "' in shader program of ID " << shaderProgramID << "\n";
        }
