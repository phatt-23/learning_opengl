...
vec4 directionalLight(
    ...
    float isVisible = when_gt(dot(normal, viewDirection), 0.0);

    float diffuseFactor = max(dot(normal, lightDirection), 0.0f) * isVisible;
    ...
    float specularFactor = specularAmount * specularStrength * isVisible;
