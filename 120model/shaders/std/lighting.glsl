...
vec4 directionalLight(
    ...
    outColor = texture2D(diffuseTexture2D, textureCoordinatesVec2) * (ambience + diffussion);
    outColor += texture2D(specularTexture2D, textureCoordinatesVec2).r * speculant;
...
vec4 pointLight(
    ...
    outColor = texture2D(diffuseTexture2D, textureCoordinatesVec2) * (diffussion * lightAttenuation + ambience);
    outColor += texture2D(specularTexture2D, textureCoordinatesVec2).r * (speculant * lightAttenuation);
...
vec4 spotLight(
    ...
    vec4 outColor;
    outColor = texture2D(diffuseTexture2D, textureCoordinatesVec2) * (diffussion * spotLightIntensity * lightAttenuation + ambience);
    outColor += texture2D(specularTexture2D, textureCoordinatesVec2).r * (speculant * spotLightIntensity * lightAttenuation);
    outColor *= lightSourceColorVec4;
