...
export class ShaderProgram {
    ...
    static auto parseShaderSourceWithIncludes(const std::string& filePath) -> std::string {
        // std::cout << "Parsing include file '" << filePath << "'" << "\n";
        ...
        // std::cout << "Base path of '" << filePath << "' is: " << basePath << "\n";

        while (std::getline(stream, line)) {
            if (line.rfind("#include", 0) == 0) {
                std::string includeFile = line.substr(9);
                includeFile = basePath + includeFile.substr(1, includeFile.size() - 2);
                // std::cout << "Including '" << includeFile << "' into '" << filePath << "' source file.\n";
                ss << "/// Begin include from '" << includeFile << "'.\n"
                    << parseShaderSourceWithIncludes(includeFile) 
                    << "/// End include from '" << includeFile << "'.\n\n";
            } else {
                ss << line << "\n";
            }
        }
            ...
            if (line.find("#shader") != std::string::npos) {
                ...
                if (line.rfind("#include", 0) == 0) {
                    ...
                    ss[static_cast<int>(type)]  
                        << "/// Begin include from '" << includeFile << "'.\n"
                        << parseShaderSourceWithIncludes(includeFile) 
                        << "/// End include from '" << includeFile << "'.\n\n";
                } else {
